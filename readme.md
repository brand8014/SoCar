...
서비스를 이용했던 내역과 예약내역 등을 실시간으로 확인할 수 있다.
또한 관리자가 고객 정보를 직접 수정하고 등록, 삭제가 가능함으로써 효율이 높아짐
### 2 차량정보
프로그램에 등록된 차량의 종류, 번호,위치, 대여가능 여부, 해당 차의 파손/사고 여부 등 구체적인 사항들을
실시간으로 조회 할 수 있다.
또한 수정, 등록, 삭제 기능을 이용해 새로운 차량의 정보와 적용되는 보험, 이벤트 정보 등을 조절할 수 있어 
실시간으로 유연하게 관리가 가능하다.
### 3 이벤트 및 기타 
고객들에게 제공되는 이벤트와 보험의 종류에 따라 달라지는 할인료와 보험료의 내역을 확인할 수 있고
새로운 이벤트를 추가하거나 기존의 이벤트를 수정, 삭제하는 기능이 가능하다.
이 밖에도 차량 대여 서비스를 이용할 수 있는 지역과 그에 따른 대여정보도 실시간으로 확인하고 수정이 가능하다.
